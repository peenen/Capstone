...
def smote_sampling(df):
    ...
    """
    使用 SMOTE 方法对数据集进行过采样，增强少数类样本。
    假设目标字段为 'user_id'，并且有其他特征可用于构造特征矩阵。
    """

    # 构造特征和标签
    X = df.drop(columns=["rating", "timestamp"])  # 特征列（如 user_id, item_id 等）
    y = df["rating"]  # 标签（可以是离散评分等级）

    # 使用 SMOTE 进行过采样
    smote = SMOTE(random_state=42)
    X_res, y_res = smote.fit_resample(X, y)

    # 合并回 rating 和 timestamp（timestamp 不参与采样，需补全）
    resampled_df = pd.DataFrame(X_res, columns=X.columns)
    resampled_df["rating"] = y_res
    resampled_df["timestamp"] = int(pd.Timestamp.now().timestamp())  # 可选填充当前时间戳
